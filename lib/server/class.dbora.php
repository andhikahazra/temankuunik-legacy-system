...
class DBORA {
    ...
    * @return resource | false
    */
    public function getRowsCount($sql, $bind = false) {
        $stmt = $this->select("select count(*) as cnt from ($sql)  ", $bind);
        $row = $this->FetchArray($stmt);
        return $row['CNT'];
    }
    ...
    */
    ...
    /**
    * select command wrapper with Limiter
    *
    * @param string $sql the query text
    * @param array | false $bind array of pairs binding variables
    ...
    * Added by Ebta
    */
    public function SelectLimit($sql, $bind = false, $start=1, $limit=15) {
        //$end = $limit * $page;
        //$start = $end - $limit + 1;
        $end = $start + $limit;
        ...
        $modSql = "
                  select * from (
                  select xyz.*, ROWNUM rnum from ($sql) xyz where rownum <= $end
                  ) where rnum >= $start";
        return $this->Execute($modSql, $bind);
    }
    ...
    */
    public function FreeStatement($stid) {
        unset($this->statements[(int)$stid]);
    ...
    */
    public function QuerySnapshot($stid = false) {
        if ($stid) return $this->statements[(int)$stid]['text'];
