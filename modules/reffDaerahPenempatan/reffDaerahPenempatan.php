...
class reffDaerahPenempatan extends Database
{
    ...
    {
        $findField = [
            "id_reff_mekanisme_penempatan","daerah_penempatan","keterangan"];
        return $findField;
    }
    ...
    public function PUBLIC_list($return = false)
    {
        ...
        $sql = "SELECT id_reff_daerah_penempatan, id_reff_mekanisme_penempatan, mp.mekanisme_penempatan, daerah_penempatan, dp.keterangan
                FROM reff_daerah_penempatan dp
                LEFT JOIN reff_mekanisme_penempatan mp USING (id_reff_mekanisme_penempatan)";
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM reff_daerah_penempatan dp LEFT JOIN reff_mekanisme_penempatan mp USING (id_reff_mekanisme_penempatan)";
    ...
    public function PUBLIC_listPrint($return = false)
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $userId = $this->get_userId();
        $sql = 'SELECT * FROM reff_daerah_penempatan';
        if (isset($_POST['search']['value']) && $_POST['search']['value'] != '') {
            $keywords = strtolower($_POST['search']['value']);
            $findField = $this->findField();
            $criteria = $this->buildSqlSearchingCriteria($keywords, $findField);
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "insert into reff_daerah_penempatan (id_reff_mekanisme_penempatan,daerah_penempatan,keterangan) VALUES 
                                          (:id_reff_mekanisme_penempatan,:daerah_penempatan,:keterangan)";
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "update reff_daerah_penempatan set id_reff_mekanisme_penempatan=:id_reff_mekanisme_penempatan,daerah_penempatan=:daerah_penempatan,keterangan=:keterangan
                 WHERE id_reff_daerah_penempatan=:id_reff_daerah_penempatan";
        echo $this->dbDataExecute($sql, $params['data']);
    }
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "delete from reff_daerah_penempatan WHERE id_reff_daerah_penempatan=:id_reff_daerah_penempatan";
        echo $this->dbDataExecute($sql, $params['data']);
    }
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "SELECT id_reff_mekanisme_penempatan, mekanisme_penempatan, keterangan FROM reff_mekanisme_penempatan";
