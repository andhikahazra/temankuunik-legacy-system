...
class ReffKecamatan extends Database
{
    ...
    public function PUBLIC_list($return = false)
    {
        ...
        $sql = 'SELECT a.*, b.kabupaten, c.provinsi FROM reff_kecamatan a
                LEFT JOIN  reff_kabupaten b
                ON a.id_reff_kabupaten=b.id_reff_kabupaten
                LEFT JOIN reff_provinsi c
                ON a.id_reff_provinsi=c.id_reff_provinsi';
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM reff_kecamatan ";
    ...
    public function PUBLIC_listPrint($return = false)
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $userId = $this->get_userId();
        $sql = 'SELECT * FROM reff_kecamatan';
        if (isset($_POST['search']['value']) && $_POST['search']['value'] != '') {
            $keywords = strtolower($_POST['search']['value']);
            $findField = $this->findField();
            $criteria = $this->buildSqlSearchingCriteria($keywords, $findField);
     ...
     public function PUBLIC_add()
    {
        ...
        $sql = "INSERT into reff_kecamatan (KECAMATAN,
                                            id_reff_provinsi,
                                            id_reff_kabupaten,
                                            user_input,
                                            tgl_input) VALUES 
    ...
    public function PUBLIC_update()
    {
        ...
        $sql = "UPDATE reff_kecamatan set KECAMATAN=:KECAMATAN,
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "delete from reff_kecamatan WHERE id_reff_kecamatan=:id_reff_kecamatan";
        echo $this->dbDataExecute($sql, $params['data']);
    }
