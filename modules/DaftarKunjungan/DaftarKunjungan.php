...
class DaftarKunjungan extends Database
{
    ...
    private function get_userUnitKerja()
    {
        $user = new os;
        $userUnit = $user->getUserUnit();
        return $userUnit['kode_unit_kerja'];
    }
    ...
    public function PUBLIC_list($return = false)
    {
        ...
        $kode_unit_kerja = $this->get_userUnitKerja();
        $sql = 'SELECT *, date(k.tgl_input) tanggal FROM kunjungan k left join tamu t on k.id_tamu = t.id ';
        $where = " where opd='$kode_unit_kerja' ";
        ...
        if ((isset($_POST['startDate']) && $_POST['startDate'] != '') && (isset($_POST['endDate']) && $_POST['endDate'] != '')) {
            ...
            $where .= " and " . "DATE(k.tgl_input) BETWEEN '$startDate' AND '$endDate'";
        }
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM kunjungan k left join tamu t on k.id_tamu = t.id " . $where;
