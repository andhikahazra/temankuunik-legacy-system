...
class ReffStatus extends Database
{
    ...
    public function PUBLIC_list($return = false)
    {
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM reff_status ";
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "insert into reff_status (status) VALUES 
                                          (:status)";
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "update reff_status set status=:status
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "delete from reff_status WHERE id_status=:id_status";
        echo $this->dbDataExecute($sql, $params['data']);
    }
