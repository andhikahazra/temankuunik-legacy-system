...
(function() {
    ...
    MyApp.$me.selectorTable = $('#datatable_fixed_column').DataTable({
        ...
        initComplete: function() {
            ...
            selectortable.on('click', '.btUpdate', function() {
                ...
                domcbLapanganUsaha.select2().select2('val', dataRow.id_reff_lapangan_usaha);
            });
            ...
            selectortable.on('click', '.btHapus', function() {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                konfirmasi = confirm('Apakah anda yakin ingin menghapus data ' + dataRow.nama_usaha);
                if (konfirmasi) {
                    hapus(dataRow);
                }
            });
    ...
    MyApp.ajax(param2).done(function(resp) {
        domcbLapanganUsaha.html('<option value="0" disabled selected>- Pilih Data Lapangan Usaha -</option>');
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function(tpl) {
                    ...
                    for (i = 0; i < resp.result.length; i++) {
                        data2[i] = {};
                        data2[i].id_reff_nama_usaha = nama = resp.result[i].id_reff_nama_usaha;
                        data2[i].id_reff_lapangan_usaha = nama = resp.result[i].id_reff_lapangan_usaha;
                        data2[i].nama_usaha = nama = resp.result[i].nama_usaha;
