...
class ReffNamaUsaha extends Database
{
    ...
    {
        $findField = [
            "id_reff_lapangan_usaha", "nama_usaha", "kode"
        ];
        return $findField;
    }
    ...
    public function PUBLIC_getLapanganUsaha()
    {
        ...
        $sql = "SELECT id_reff_lapangan_usaha, lapangan_usaha, kode
    ...
    public function PUBLIC_list($return = false)
    {
        ...
        $sql = 'SELECT nu.nama_usaha, nu.kode, nu.id_reff_nama_usaha, lu.kode kode_lapangan_usaha, lu.lapangan_usaha FROM reff_nama_usaha nu LEFT JOIN reff_lapangan_usaha lu USING(id_reff_lapangan_usaha)';
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM reff_nama_usaha ";
    ...
    public function PUBLIC_listPrint($return = false)
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $userId = $this->get_userId();
        $sql = 'SELECT * FROM reff_nama_usaha';
        if (isset($_POST['search']['value']) && $_POST['search']['value'] != '') {
            $keywords = strtolower($_POST['search']['value']);
            $findField = $this->findField();
            $criteria = $this->buildSqlSearchingCriteria($keywords, $findField);
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "insert into reff_nama_usaha (id_reff_lapangan_usaha,nama_usaha,kode) VALUES 
                                          (:id_reff_lapangan_usaha,:nama_usaha,:kode)";
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "update reff_nama_usaha set id_reff_lapangan_usaha=:id_reff_lapangan_usaha,nama_usaha=:nama_usaha,kode=:kode
                 WHERE id_reff_nama_usaha=:id_reff_nama_usaha";
        echo $this->dbDataExecute($sql, $params['data']);
    }
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "delete from reff_nama_usaha WHERE id_reff_nama_usaha=:id_reff_nama_usaha";
        echo $this->dbDataExecute($sql, $params['data']);
    }
