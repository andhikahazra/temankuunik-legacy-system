...
(function() {
    ...
    MyApp.$me.selectorTable = $('#datatable_fixed_column').DataTable({
        ...
        "columns": [{
            ...
            /* { "data": "id_reff_desa" }, { "data": "ID_PROP" }, { "data": "ID_KAB" }, { "data": "ID_KEC" }, { "data": "ID_KEL" }, */
            { "data": "provinsi" },
            { "data": "kabupaten" },
            { "data": "KECAMATAN" },
            { "data": "KELURAHAN" },
        ...
        initComplete: function() {
            ...
            selectortable.on('click', '.btUpdate', function() {
                ...
                // domcbKecamatan.select2().select2('val', dataRow.id_reff_kecamatan);
                load_kabupaten(dataRow.id_reff_kabupaten, dataRow.id_reff_kecamatan);
                load_kecamatan(dataRow.id_reff_kecamatan);
            });
            ...
            selectortable.on('click', '.btHapus', function() {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                konfirmasi = confirm('Apakah anda yakin ingin menghapus data ' + dataRow.KELURAHAN);
                if (konfirmasi) {
                    hapus(dataRow);
                }
            });
    ...
    //     domcbKecamatan.html('<option value="0" disabled selected>- Pilih Data Kecamatan -</option>');
    ...
    var load_kecamatan = function(value = 0) {
        var param2 = {
            option: 'PUBLIC',
            action: 'getKecamatan',
        ...
        domcbKecamatan = $('.cbKecamatan');
        MyApp.ajax(param2).done(function(resp) {
            domcbKecamatan.html('<option value="0" selected>- Pilih Data -</option>');
            $.each(resp.result, function(index, val) {
                domcbKecamatan.append('<option value="' + val.id_reff_kecamatan + '">' + val.KECAMATAN + '</option>');
            });
