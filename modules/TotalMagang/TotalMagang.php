...
class TotalMagang extends Database
{
    ...
    public function PUBLIC_list($return = false)
    {
        ...
        if ($x == 0) {
            $x = date('m');
            $sql = "select bidang,count(nama) jumlah from magang m where month(m.startDate)<=$x and month(m.endDate)>=$x group by bidang";
        } else {
            $sql = "select bidang,count(nama) jumlah from magang m where month(m.startDate)<=$x and month(m.endDate)>=$x group by bidang";
        }
    ...
    public function PUBLIC_UpdateDataFormat2()
    {
        $params = isset($_GET) ? $_POST : $_POST;
        // print_r($params);exit;
        $tahun = $params['data']['thun'];
    ...
    public function PUBLIC_instansi()
    {
        $params = isset($_GET) ? $_GET : $_POST;
