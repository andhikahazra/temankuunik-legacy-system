...
class DataPendaftar extends Database
{
    ...
    private function findField() {
        $findField = [
            "nik","nama","alamat","email","no_hp","input_date","update_date","user_update","smartphone","minat","kecamatan"
        ];
        return $findField;
    }
    ...
    public function PUBLIC_list($return=false)
    {
        ...
        if (isset($_POST['order'])) {
            ...
            $sql .= ($_POST['order'][0]['column']==7)?" order by kecamatan $ascDesc ":'';
            $sql .= ($_POST['order'][0]['column']==5)?" order by tanggal_lahir $ascDesc ":'';
        }
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM pendaftar ";
    ...
    public function PUBLIC_add(){
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "insert into pendaftar (nik,nama,alamat,email,no_hp,input_date,update_date,user_update,smartphone,minat) VALUES 
                                          (:nik,:nama,:alamat,:email,:no_hp,:input_date,:update_date,:user_update,:smartphone,:minat)";
...
//Close and output PDF document
        $pdf->Output('export_pdf_file.pdf', 'I');
    }

    public function PUBLIC_presensi()
    {
        $data = $this->PUBLIC_list(true);
        foreach ($data as $idx=>$val){
            $data[$idx]->no = $idx+1;
        }
        $data['value'] = $data;
        $i = 0;
        $data['judul'] = "Presensi Peserta Bimtek";
         ...
         * */
//        print_r($data);exit;
        $pdf = $this->createHtml2Pdf(null, $data, 'tpl_pdf_presensi.html');
        // set document information
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('Arkan Herawan');
