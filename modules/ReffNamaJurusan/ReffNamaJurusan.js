...
(function() {
        ...
        initComplete: function() {
            ...
            $('.btTop').click(function() {
                ...
                if (aksi === 'add') {
                    ...
                    $('#myModal').modal('show');
                    $('#form_input')[0].reset();
                    $('.cbTingkatPendidikan').val(null).trigger('change');
                    $('.cbJurusanPendidikan').val(null).trigger('change');
            ...
            selectortable.on('click', '.btUpdate', function() {
                ...
                domcbTingkatPendidikan.select2().select2('val', dataRow.id_reff_tingkat_pendidikan);
                domcbJurusanPendidikan.select2().select2('val', dataRow.id_reff_jurusan_pendidikan);
            });
            ...
            selectortable.on('click', '.btHapus', function() {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                konfirmasi = confirm('Apakah anda yakin ingin menghapus data ' + dataRow.nama_jurusan);
                if (konfirmasi) {
                    hapus(dataRow);
                }
            });
    ...
    var param2 = {
        option: 'PUBLIC',
        action: 'getTingkatPendidikan'
    };
    domcbTingkatPendidikan = $('.cbTingkatPendidikan');
    MyApp.ajax(param2).done(function(resp) {
        MyApp.preventLoad = true;
        domcbTingkatPendidikan.html('<option value="0" disabled selected>- Pilih Data Tingkat Pendidikan -</option>');
    ...
    var load_jurusan = function() {
        /*load jurusan pendidikan di database*/
        var param2 = {
            option: 'PUBLIC',
        ...
        MyApp.ajax(param2).done(function(resp) {
            domcbJurusanPendidikan.html('<option value="0" disabled selected>- Pilih Data Jurusan Pendidikan -</option>');
            $.each(resp.result, function(index, val) {
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function(tpl) {
                    ...
                    for (i = 0; i < resp.result.length; i++) {
                        data2[i] = {};
                        data2[i].id_reff_nama_jurusan = nama = resp.result[i].id_reff_nama_jurusan;
                        data2[i].id_reff_jurusan_pendidikan = nama = resp.result[i].id_reff_jurusan_pendidikan;
                        data2[i].id_reff_tingkat_pendidikan = nama = resp.result[i].id_reff_tingkat_pendidikan;
