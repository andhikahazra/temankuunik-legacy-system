    ...
    MyApp.$me.selectorTable = $('#datatable_fixed_column').DataTable({
        ...
        initComplete: function() {
            ...
            selectortable.on('click', '.btHapus', function() {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                konfirmasi = confirm('Apakah anda yakin ingin menghapus data ' + dataRow.nama_badan_usaha);
                if (konfirmasi) {
                    hapus(dataRow);
                }
            });
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function(tpl) {
                    ...
                    for (i = 0; i < resp.result.length; i++) {
                        data2[i] = {};
                        data2[i].id_reff_badan_usaha = nama = resp.result[i].id_reff_badan_usaha;
                        data2[i].nama_badan_usaha = nama = resp.result[i].nama_badan_usaha;
                    }
...
//# sourceURL=ReffJaminanSosial.js