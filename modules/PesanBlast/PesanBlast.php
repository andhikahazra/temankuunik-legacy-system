...
class PesanBlast extends Database
{
    ...
    public function save_data()
    {
        ...
        // Mengambil id kategori nomor telepon
        $id_kategori = $data[0]['value'];

        // Mengambil isi pesan
        $isi_pesan = $data[1]['value'];

         // Mengambil isi footer
        $isi_footer = $data[2]['value'];
        ...
        //MULAI LOOPING PERULANGAN PENGIRIMAN PESAN DI KATEGORI

        $sql  = "SELECT no_telp,nama FROM tb_phonebook WHERE id_kategori = '$id_kategori'";
        $value = $this->dbFwSelectAndReturnAll($sql,null,true);

        foreach ($value as $item) {
            $nomor_telepon = $item->no_telp;
            $nama_kontak = $item->nama;
            // echo $item->no_telp . "<br>";


                // Data yang akan dikirim
                $data = [
                    'phone' => $nomor_telepon,
                    'message' => "Kepada Yth : ".$nama_kontak . "\n\n".$isi_pesan . "\n\n" . $isi_footer
                ];
