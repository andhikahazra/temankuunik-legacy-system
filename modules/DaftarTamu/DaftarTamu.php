...
class DaftarTamu extends Database
{
    ...
    public function PUBLIC_list($return = false)
    {
        ...
        if ((isset($_POST['startDate']) && $_POST['startDate'] != '') && (isset($_POST['endDate']) && $_POST['endDate'] != '')) {
            ...
            $where .= " and " . "DATE(tgl_input) BETWEEN '$startDate' AND '$endDate'";
        }
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM tamu ";
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "insert into tamu (nama,no_hp,email,instansi,tgl_input,tgl_edit) VALUES 
                                          (:nama,:no_hp,:email,:instansi,:tgl_input,:tgl_edit)";
    ...
    {
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "update tamu set nama=:nama,no_hp=:no_hp,email=:email,instansi=:instansi,tgl_input=:tgl_input,tgl_edit=:tgl_edit
                 WHERE id=:id";
        echo $this->dbDataExecute($sql, $params['data']);
    }
