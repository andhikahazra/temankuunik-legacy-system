...
(function() {
    ...
    MyApp.$me.selectorTable = $('#datatable_fixed_column').DataTable({
        ...
        "columns": [{
            ...
            },
            {
                ...
                width: '8%'
            }, { "data": "kategori" }, { "data": "deskripsi" }, { "data": "id_unit_layanan" },
        ],
        dom: 'Bfrtip',
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function(tpl) {
                    ...
                    for (i = 0; i < resp.result.length; i++) {
                        data2[i] = {};
                        data2[i].id_kategori = nama = resp.result[i].id_kategori;
                        data2[i].kategori = nama = resp.result[i].kategori;
                        data2[i].deskripsi = nama = resp.result[i].deskripsi;
                        data2[i].id_unit_layanan = nama = resp.result[i].id_unit_layanan;
                    }
    ...
    MyApp.ajax(params).done(function(resp) {
        ...
        $.each(resp.result, function(idx, val) {
            $('#cbUnitlayanan').append('<option value="' + val.id_unit_layanan + '">' + val.unit_layanan + '</option>');
        })
    })
