    ...
    MyApp.$me.selectorTable = $('#datatable_fixed_column').DataTable({
        ...
        initComplete: function() {
            ...
            selectortable.on('click', '.btHapus', function() {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                konfirmasi = confirm('Apakah anda yakin ingin menghapus data ' + dataRow.bahasa_asing);
                if (konfirmasi) {
                    hapus(dataRow);
                }
            });
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function(tpl) {
                    ...
                    for (i = 0; i < resp.result.length; i++) {
                        data2[i] = {};
                        data2[i].id_reff_bahasa_asing = nama = resp.result[i].id_reff_bahasa_asing;
                        data2[i].bahasa_asing = nama = resp.result[i].bahasa_asing;
                    }
